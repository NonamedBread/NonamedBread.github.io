...
```Javascript
  const [postPerPage] = useState(13) //페이지당 리스트 수 (한 페이에 13개씩 보여줌)
...
- 글을 쓰면서 코드를 둘러보니 컴포넌트간 객체를 넘겨주는 부분에서 엔드포인트를 객체로 넘겨 주었으면 환경에 따라서 엔드포인트를 변경 했어야 됐는데(Swagger, Local) 좀 더 깔끔하게 사용할 수 있었을것 같다는 생각이 들었다. 
- API통신할 때 null 값을 slice 할때 오류가 발생하는 부분과 Pagination 부분은 잘 기억해 둬야 겠다
